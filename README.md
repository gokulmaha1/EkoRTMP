...
### 5. Customization
- **Overlay**: Edit `overlay.html` and `style.css` locally, then rebuild the Docker image to apply changes.
- **Pipeline**: Edit `main.py` if you want to change the video source (currently a test ball pattern) to a file or RTSP stream.
